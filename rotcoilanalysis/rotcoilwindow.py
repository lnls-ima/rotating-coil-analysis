...
class MainWindow(_QMainWindow):
        ...
        if (self.ui.cb_spec_on.currentIndex() == 0 and
           all(x == magnet_names[0] for x in magnet_names)):
            ...
            if sys_residue is not None:
                ...
                residue['Systematic'] = _pd.Series(sys_residue, index=index)
                residue['Upper limit'] = _pd.Series(max_residue, index=index)
                residue['Lower limit'] = _pd.Series(min_residue, index=index)
    ...
    def _plot_residual_multipoles(self, all_files=False):
        ...
        residual_mult_skew = _np.zeros([max_harmonic, xnpts, len(index_list)])
