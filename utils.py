    ...
    def set_callbacks(self):
        ...
        def button_press_callback(event):
            if event.x is not None and event.y is not None:
                dist = self.get_distance_from_point(event.x, event.y)
                if dist < self.tol:
        ...
        def motion_notify_callback(event):
            if not self.change_position: return
            if event.xdata is None and event.ydata is None: return
            self.set_center_position(event.xdata, event.ydata)
            self.canvas.draw()

        self.canvas.mpl_connect('button_press_event', button_press_callback)
        self.canvas.mpl_connect('button_release_event', button_release_callback)
        self.canvas.mpl_connect('motion_notify_event', motion_notify_callback)
...
def scientific_notation(value, error):
    ...
    scientific_notation = '(' + value_str + " " + chr(177) + " " + error_str +')'+ exponent_str
